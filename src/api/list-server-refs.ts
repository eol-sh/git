/**
 * @fileoverview Git list-server-refs API - High-level user interface
 *
 * This module provides the public API for list-server-refs operations in the Git implementation.
 * It handles parameter validation, file system adaptation, and error handling.
 *
 * @module api/list-server-refs.ts
 * @version 1.0.0
...
import { GitRemoteHTTP } from "../managers/git-remote-http.ts";
import { parseListRefsResponse } from "../wire/parse-list-refs-response.ts";
import { writeListRefsRequest } from "../wire/write-list-refs-request.ts";
 ...
 *
 * Using @eol/git in a browser, with a CORS proxy, listing only the branches (refs/heads) of https://eol.sh/~eol/git
 * - Protocol Version 1 took ~300ms and transferred 84 KB.
 * - Protocol Version 2 took ~500ms and transferred 4.1 KB.
 *
 * Using @eol/git in a browser, with a CORS proxy, listing only the branches (refs/heads) of https://gitlab.com/gitlab-org/gitlab
...
export async function listServerRefs({
  ...
  try {
    ...
    if ((remote as any).protocolVersion === 1)
      return formatInfoRefs(remote as any, prefix, symrefs, peelTags);
    ...
    (err as any).caller = "git.listServerRefs";
    throw err;
  }
}
