...
/**
 * EOL Git HTTP Server - Simplified Version
 *
 * Uses the simplified API that doesn't require manual filesystem configuration.
 * This version is much cleaner and easier to use!
 */
...
import { createFileSystem } from "../src/index.ts";
...
class GitServer {
  ...
  private fs: any; /*** FileSystem instance ***/

  constructor(config: GitServerConfig) {
    this.config = config;
    this.fs = createFileSystem(); /*** Create once, use everywhere! ***/
  }
  ...
  async cloneRepository(repo: string, body: any): Promise<ApiResponse> {
    const { depth, ref = "primary",  url: cloneUrl } = body;
  ...
  async handleRequest(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/health") {
        result = this.createResponse(true, { status: "healthy", version: "2.0.0-simplified" });
  ...
  async start(): Promise<void> {
    ...
    console.log(`🚀 EOL Git Server (Simplified) running on http://localhost:${this.config.port}`);
    console.log(`🎯 Using simplified API - no filesystem configuration needed!`);
    ...
    console.log("\n✨ Example: Initialize a repo");
    console.log(`  curl -X POST http://localhost:${this.config.port}/repos/my-project \\`);
    console.log(`    -H "Content-Type: application/json" \\`);
    console.log(`    -d '{"operation": "init", "defaultBranch": "main"}'`);
