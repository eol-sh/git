...
/**
 * EOL Git HTTP Server - JSR Version
 *
 * A Deno HTTP server that wraps @eol/git (from JSR) to provide Git operations via REST API.
 * This server allows you to perform Git operations through HTTP requests.
 *
 * Usage with published package:
 * deno run --allow-env --allow-net --allow-read --allow-write jsr:@eol/git/examples/git-server
 */
...
class GitServer {
  ...
  async start(): Promise<void> {
    ...
    console.log(`🚀 EOL Git Server (JSR) running on http://localhost:${this.config.port}`);
    console.log(`📦 Using @eol/git from JSR`);
